...
function psinorm(psi::T, canvas::Canvas) where {T<:Real}
    psin = (psi - canvas.Ψaxis) / (canvas.Ψbnd - canvas.Ψaxis)
    if (psin < 0.0) && isapprox(psin, 0.0; atol=sqrt(eps(T)))
        return 0.0
    elseif (psin > 1.0) && isapprox(psin, 1.0)
        return 1.0
    end
    return psin
end
