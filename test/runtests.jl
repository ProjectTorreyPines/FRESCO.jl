...
@testset "FRESCO Solve" begin
    ...
    @test FRESCO.solve!(canvas, profile, 20, 3; relax=0.5, debug=true, control=:shape, tolerance=1e-4, fixed_coils=25:48, initialize_current=true) == 0
    canvas.λ_regularize = 0.0 # turn off regularization for the next test
    @test FRESCO.solve!(canvas, profile, 10, 3; relax=0.5, debug=true, control=:shape, tolerance=1e-3, initialize_current=true) == 1 # won't converge in iterations
